...
) ENGINE=InnoDB AUTO_INCREMENT=7481 DEFAULT CHARSET=utf8;


-- ----------------------------
-- Table structure for activationcode
-- ----------------------------
...
INSERT INTO `menu` VALUES ('15', '登陆公告', 'notice/notice?mainView=true', '12');
...
INSERT INTO `menu` VALUES ('30', '活动', 'activity/list?mainView=true', '-1');
...
CREATE TABLE `crystal_detailed_summary` (
  ...
  `server_id` int(11) DEFAULT NULL COMMENT '服务器id',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8 COMMENT='# 水晶明细汇总';
...
CREATE TABLE `crystal_level` (
  ...
  `server_id` int(11) DEFAULT NULL COMMENT '服务器id',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=17 DEFAULT CHARSET=utf8 COMMENT='# 水晶产出&消耗&留存 (等级汇总)';
...
CREATE TABLE `crystal_use_summary` (
  ...
  `server_id` int(11) DEFAULT NULL COMMENT '服务器id',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=utf8 COMMENT='######### 水晶使用汇总 ########';
...
CREATE TABLE `fb_summary` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=utf8 COMMENT='# 副本挑战汇总\r\n';
...
CREATE TABLE `log_db` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COMMENT='# 日志 db ';
...
CREATE TABLE `money_summary` (
  ...
  `server_id` int(11) DEFAULT NULL COMMENT '服务器id',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=utf8 COMMENT='# 货币(产出&消耗)汇总';
...
CREATE TABLE `ns_dau_summary` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=47 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT='# 新服活跃汇总表';
...
CREATE TABLE `ns_income_summary` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=47 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT='# 新服收入汇总表';
...
) ENGINE=MyISAM AUTO_INCREMENT=1280 DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `order_temp` (
  ...
  `platform` varchar(16) NOT NULL COMMENT '平台标识,玩家账号注册时的平台',
  PRIMARY KEY (`order_id`)
) ENGINE=MyISAM AUTO_INCREMENT=4691 DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `pay_lty` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=650 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT='### PAY LTV  30 日支付LVT ';
...
CREATE TABLE `rmb_crystal` (
  ...
  `server_id` int(255) DEFAULT NULL COMMENT '服务器id',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=utf8 COMMENT='## 人民币购买水晶消费档次统计';
...
CREATE TABLE `summary_sql` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=18 DEFAULT CHARSET=utf8 COMMENT='# 汇总 sql';
...
-- 新增充值人数
set @newCount = 0;
select sum(amount) into @newCount from `order` where UNIX_TIMESTAMP(create_time)>=@today and  UNIX_TIMESTAMP(finish_time)<=@tomorrow and `order`.platform LIKE platfrom0;


-- 总充值用户
set @totalUser = 0;
set @totalCount = 0;
select count(distinct account_id) into @totalUser from `order` where UNIX_TIMESTAMP(finish_time)<=@tomorrow and `order`.platform LIKE platfrom0;
select sum(amount) into @totalCount from `order` where UNIX_TIMESTAMP(finish_time)<=@tomorrow and `order`.platform LIKE platfrom0;


-- 当日总充值用户
set @totalDayUser = 0;
-- set @totalDayCount = 0;
select count(distinct account_id) into @totalDayUser from `order` where finish_time LIKE @todayStr and `order`.platform LIKE platfrom0;
...
-- 通过算出前天充值总用户
set @totalUser0 = 0;
select count(distinct account_id) into @totalUser0 from `order` where UNIX_TIMESTAMP(finish_time)<=@today and `order`.platform LIKE platfrom0;
...
-- 统计acu & pcu 
INSERT INTO acu_pcu_summary(pcu,acu,server_id,t_time)
SELECT
...
BEGIN
set @total_count = 0;
select sum(amount) into @total_count from `order`;

set @today = date(now());

set @today_count = 0;
SELECT sum(amount) into @today_count FROM `order` where finish_time LIKE concat(@today, "%");
