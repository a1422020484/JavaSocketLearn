 ...
 * @author zhuangyuetao
 */
public class YijieTool {
	public static final String flag = "yijie";
	...
	public static LoginResponse login(Map<String, String> params) {
		...
		if (log.isDebugEnabled()) {
			...
			log.debug("yijie|Yijie LoginVerify sdk:{},uin:{},sess:{}|YijieAppId:{}", sdk, uin, sess, YijieAppId);
		}

		String resp = null;
		try {
			resp = HttpUtils.create(YijieVerifyUrl).urlDecode(false).addParam("sdk", sdk).addParam("app", YijieAppId)
					.addParam("uin", uin).addParam("sess", sess).get();
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (!"0".equals(resp)) {
			log.debug("yijie|login resp:{}", resp);
