...
@Path("addedly")
public class AddedlyController {
	...
	public String serverReportTotal(Invocation inv, @Param("type") int type) throws Throwable {
		...
		List<SubPlatformModel> platforms = subPlatformManager.getSubPlatformList();
		...
		request.setAttribute("sendUrl", Utils.url(inv, "/statistics/addedly/totalInfo?type=" + type));
	...
	public String serverReportTotalInfo(Invocation inv, @Param("type") int type, @Param("channelId") String channelId, @Param("srvId") int srvId, @Param("startTime") String startTime,
		...
		return "@" + selectTable(template, type, channelId, srvId, resultTimeHms);
	}

	private String selectTable(JdbcTemplate template, int type, String channelId,int serverId, String resultTimeHms){
		...
		sql = MessageFormat.format(temp.getSql(),resultTimeHms,platform,server_id );
