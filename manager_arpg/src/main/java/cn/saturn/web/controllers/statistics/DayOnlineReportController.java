 ...
 *
 */
@Path("dayOnlineReport")
public class DayOnlineReportController {
	@Resource
	...
	public String dayOnlineReportTotal(Invocation inv, @Param("type") int type) throws Throwable {
		...
		request.setAttribute("sendUrl", Utils.url(inv, "/statistics/dayOnlineReport/totalInfo?type=" + type));
		request.setAttribute("platforms", platforms);
		request.setAttribute("servers", servers);

		return "dayOnlineReport";
	}

	@Get("totalInfo")
	public String dayOnlineReportTotalInfo(Invocation inv, @Param("type") int type, @Param("channelId") String channelId,
		...
		return "@" + selectTable(template, type, channelId, srvId, resultTime,resultQuery);
	}

	private String selectTable(JdbcTemplate template, int type, String channelId, int serverId, String resultTime,String resultQuery) {
		...
		String sql = MessageFormat.format(temp.getSql(), resultQuery,resultTime, server_id, platform);
