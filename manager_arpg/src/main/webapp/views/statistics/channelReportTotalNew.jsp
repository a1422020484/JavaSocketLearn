...
<div id="body2" 
    ...
    <div id="tb" style="padding:2px 5px;">
     	区服:
		<input name="srvId" id="srvId" class="easyui-combobox"  style="width:150px"
    	data-options="valueField:'id',textField:'name'"/>
    	渠道:
    	<input name="platform" id="platform" class="easyui-combobox"  style="width:150px"
    	data-options="valueField:'id',textField:'name'"/>
    	子渠道:
    	<input name="subPlatform" id="subPlatform" class="easyui-combobox"  style="width:150px"
    	data-options="valueField:'id',textField:'name'"/>
	...
	function changePlatform(){
		 ...
		 $.ajax({type : "POST",
		    	url : "../platform/getSelect2SubPlatformEasyUi?platfrom="+platform,
		    	datatype : "json",
		    	success : function(data) {
		    	var obj = eval(data);
		    	if (obj.length > 0) {$('#subPlatform').combobox('loadData', obj);}
					$('#subPlatform').combobox('setValue', '-1');
		    	},
		    });
		// $('#subPlatform').combobox('loadData', obj);
	}
	...
	$(document).ready(function() {
		...
		$.ajax({type : "POST",
	    	url : "../platform/getSelect2PlatformEasyUi",
	    	datatype : "json",
	    	success : function(data) {
	    	var obj = eval(data);
	    	if (obj.length > 0) {$('#platform').combobox('loadData', obj);}
	    	$('#platform').combobox('setValue', '-1');
	    	},
	    });
		
		$.ajax({type : "POST",
	    	url : "../../server/getSelect2EasyUi",
	    	datatype : "json",
	    	success : function(data) {
	    	var obj = eval(data);
	    	if (obj.length > 0) {$('#srvId').combobox('loadData', obj);}
	    	$('#srvId').combobox('setValue', '-1');
	    	},
	    });
		
		$('#platform').combobox({onSelect:function(record){changePlatform();}});
	...
	function toSubmit() {
		...
		var platform = $('#platform').combobox('getValue');
		var subPlatform = $('#subPlatform').combobox('getValue');
		var srvId = $('#srvId').combobox('getValue');
		var startTime = $('#startTime').datebox('getValue');
		var endTime = $('#endTime').datebox('getValue');
		subPlatform = subPlatform== '' ? -1: subPlatform;
		
		// 通过 ajax 请求数据
		var urlStr = "${sendUrl}&platform=" + platform + "&srvId="
				+ srvId + "&subPlatform="+subPlatform+"&startTime=" + startTime + "&endTime=" + endTime
				+ "&isSelect=1";
