...
<%@ page import="cn.saturn.web.controllers.statistics.SystemLogClock"%>
...
<script type="text/javascript">
	function toCheck(dayStr, srvId) {
		...
		var retStr = "${sendUrl}/toAdd?dayStr=" + dayStr + "&srvId=" + srvId
				+ "&type=" + type;
	...
	function toDelete(dayStr, srvId) {
		load();
		
		var type  = $("select[name=type] option:selected").val();
		var retStr = "${sendUrl}/toDelete?dayStr=" + dayStr + "&srvId=" + srvId
				+ "&type=" + type;
...
<body>
	...
	<div class="div_from_aoto" style="width: 500px;">
		<h4>定时任务失败时，手动追加详细物品日志到数据库，勿动！！</h4>
		...
		<div class="control-group">
			...
			<%
				WebUtils.createSelected(pageContext, "type", "input_select", SystemLogClock.getKeys(), 0);
			%>
		...
		<div class="control-group">
			...
			<div class="controls">
				<button class="btn btn-success" style="width: 120px;"
					onclick="toCheck(getValue('dayTime'),  getValue('srvId'))">追加</button>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<button class="btn btn-success" style="width: 120px;"
					onclick="toDelete(getValue('dayTime'),  getValue('srvId'))">删除</button>
			</div>
		</div>
