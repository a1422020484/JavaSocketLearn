...
<body>
    ...
    <div id="tb" style="padding:2px 5px;">
    	渠道:
    	<select name="channelId" id="channelId" class="easyui-combobox" panelHeight="auto" style="width:100px">
        ...
        <select name="srvId" id="srvId" class="easyui-combobox"  style="width:100px">
		 ...
		 难度:
        <select name="hardId" id="hardId" class="easyui-combobox"  style="width:100px">
			<option value="-1">--请选择--</option>
			<c:forEach items="${hardLst}" var="hard">
			<option  value="${hard.getType()}">${hard.getValue()}</option>
			</c:forEach>
		</select>
	...
	function toSubmit() {
		...
		var hardId = $('#hardId').combobox('getValue');
		var startTime = $('#startTime').datebox('getValue');
		var endTime = $('#endTime').datebox('getValue');
		
		// 通过 ajax 请求数据
		var urlStr = "${sendUrl}&channelId=" + channelId + "&srvId="
				+ srvId +"&hardId=" + hardId +"&startTime=" + startTime 
