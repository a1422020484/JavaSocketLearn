...
<div id="body2" 
    ...
    <div id="tb" style="padding:2px 5px;">
		...
		子渠道:
    	<select name="subChannelId" id="subChannelId" class="easyui-combobox"  style="width:100px">
			<option value="-1">--请选择--</option>
			<c:forEach items="${subplatforms}" var="subplatform">
			<option value="${subplatform.subPlatformId}">${subplatform.name}</option>
			</c:forEach>
		</select>
	...
	function toSubmit() {
		...
		var subChannelId = $('#subChannelId').combobox('getValue');
		var srvId = $('#srvId').combobox('getValue');
		var startTime = $('#startTime').datebox('getValue');
		var endTime = $('#endTime').datebox('getValue');
		
		// 通过 ajax 请求数据
		var urlStr = "${sendUrl}&subChannelId=" + subChannelId + "&srvId="
				+ srvId + "&startTime=" + startTime + "&endTime=" + endTime
				+ "&isSelect=1";
