...
<div id="body2" 
    ...
    <div id="tb" style="padding:2px 5px;">
    	...
    	广告渠道:
    	<select name="adplatform" id="adplatform" class="easyui-combobox" style="width:100px">
			<option value="-1">--请选择--</option>
			<c:forEach items="${adplatforms}" var="adplatform">
			<option value="${adplatform}">${adplatform}</option>
			</c:forEach>
		</select>
		
		广告子渠道:
    	<select name="adsubplatform" id="adsubplatform" class="easyui-combobox" style="width:100px">
			<option value="-1">--请选择--</option>
			<c:forEach items="${adsubplatforms}" var="adsubplatform">
			<option value="${adsubplatform}">${adsubplatform}</option>
			</c:forEach>
		</select>
	...
	function toSubmit() {
		...
		var adplatform = $('#adplatform').combobox('getValue');
		var adsubplatform = $('#adsubplatform').combobox('getValue');
		var startTime = $('#startTime').datebox('getValue');
		var endTime = $('#endTime').datebox('getValue');
		
		// 通过 ajax 请求数据
		var urlStr = "${sendUrl}&adplatform=" + adplatform + "&adsubplatform="
				+ adsubplatform + "&startTime=" + startTime + "&endTime=" + endTime
				+ "&isSelect=1";
