 ...
 */
@Component
public class WordsFilter extends ResourceLoader {
	...
	public static String filterWords(String content) {
        ...
        result = EmojiFilter.filterEmoji(result, "*");
        return result;
	}
	...
	//
	public static boolean contentCheckRegex(String chatStr){
		boolean selftalk = false;
		Pattern pattern = GameConfig.ChatContentCheckPattern;
		if(null != pattern){
			Matcher matcher = pattern.matcher(chatStr);
	        int c = 0;
	        while (true) {
	            if (matcher.find()) {
	                c++;
	        ...
	        if(GameConfig.ChatContentCheckRegexMatchs <= c){
	        	selftalk = true;
	        }
		}
		return selftalk;
