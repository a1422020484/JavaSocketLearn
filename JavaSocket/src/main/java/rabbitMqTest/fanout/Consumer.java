...
public class Consumer {
	...
	public static void doConsumer1(Connection conn) {
		...
		if (conn != null) {
			try {
				...
				channel.queueDeclare(Config.QueueName1, false, false, false, null);
				
				// 创建订阅器，并接受消息
				channel.basicConsume(Config.QueueName1, false, "", new DefaultConsumer(channel) {
	 ...
	 * 消费消息
	 */
	public static void doConsumer2(Connection conn) {
		// 创建一个连接
			...
			try {
				...
				channel.basicConsume(Config.QueueName1, false, "", new DefaultConsumer(channel) {
					...
					public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
						...
						String contentType = properties.getContentType(); // 内容类型
						String content = new String(body, "utf-8"); // 消息正文
						System.out.println("消息正文2：" + content);
						channel.basicAck(envelope.getDeliveryTag(), false); // 手动确认消息【参数说明：参数一：该消息的index；参数二：是否批量应答，true批量确认小于index的消息】
					}
				});
