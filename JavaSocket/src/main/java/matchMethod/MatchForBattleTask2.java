...
public class MatchForBattleTask2 {
    ...
    public void process() {
        ...
        Map<Integer, List<MatchGroupResult>> danGroupResult = getdanGroupResult(matchingGroupResult);
        System.err.println("一共整合出:" + matchingGroupResult.size() + "个匹配组");
        if (matchingGroupResult.isEmpty())
            return;
        int size = matchingGroupResult.size();
        Set<Long> alreadyOccupied = new HashSet<>();
        List<Integer> danKeys = new ArrayList<>();
        ...
        for (int i = 0; i < size; ++i) {
            ...
            List<MatchGroupResult> currentDanMatchGroup = danGroupResult.get(KeyValue);
            ...
            if (null == result && currenStepNum > 1) {
                ...
                List<Integer> offsetKeys = new ArrayList<>();
    ...
    public MatchResult getFinalResult(Set<Long> alreadyOccupied, List<MatchGroupResult> matchingGroupResult, MatchGroupResult tempGroup,
                                      int i, int danstep, List<MatchResult> matchResults) {
    ...
    private Map<Integer, List<MatchGroupResult>> getdanGroupResult(List<MatchGroupResult> matchingGroupResult) {
        Map<Integer, List<MatchGroupResult>> result = new HashMap<>();
        for (MatchGroupResult matchGroupResult : matchingGroupResult) {
            int indexKey = indexOf(matchGroupResult.averageScore);
            List<MatchGroupResult> tempValue = result.get(indexKey);
            if (null == tempValue) {
